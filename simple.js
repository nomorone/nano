    ...
    */
    ...
    NanoBotz.getName = (jid = '', withoutContact = false) => {
        ...
        if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
            ...
            resolve(v.name || v.subject || jidDisplayName(jid))
        })
            ...
            NanoBotz.user :
            (NanoBotz.chats[jid] || {})
        return (withoutContact ? '' : v.name) || v.subject || v.vname || v.notify || v.verifiedName || jidDisplayName(jid)
    }
 ...
 */
 exports.smsg = (NanoBotz, m, hasParent) => {
    ...
    if (m.key) {
        m.id = m.key.id
        m.isBaileys = (typeof m.id === 'string' && m.id.length === 16) || (typeof m.id === 'string' && m.id.startsWith('3EB0') && m.id.length === 12) || false
